...
To add custom templates, create a ConfigMap called `smcp-templates` in the istio-operator project. This ConfigMap will contain a collection
of templates to use. This ConfigMap must then be mounted in the operator at `/usr/local/share/istio-operator/templates/`.

=== Creating the ConfigMap
To create the ConfigMap, execute the following, substituting templates-directory for a directory containing a collection 
of *ServiceMeshControlPlane* files to use as templates. The name of the template will match the name of the file.

```
 oc create configmap --from-file=templates-directory smcp-templates -n istio-operator
```
Next, instruct the operator to use the smcp-templates ConfigMap. 

Edit the operator deployment.

```
oc edit deployment -n istio-operator
```
